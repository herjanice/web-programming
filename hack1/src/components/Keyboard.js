...
const Keyboard = ({ usedChars, handleKeyup }) => {
        ...
        <div className='Keyboard-container'>
            ...
            <div id='KBrow_1' key='KBrow_1' className='Keyboard-row'>
                {firstRowLetters && firstRowLetters.map((letter) => {
                    ...
                    return (
                        <div id = {'char_' + letter.char} key={'char_' + letter.char} className={'Keyboard-char ' + color} onClick={e=>handleKeyup(e.target.innerHTML)}>{letter.char}</div>
                    )
                })}
            </div>
            ...
            <div id='KBrow_3' key='KBrow_3' className='Keyboard-row'>
                {thirdRowLetters && thirdRowLetters.map((letter) => {
                    ...
                    return (
                        letter.char === 'Enter' ?
                            <div id={'char_' + letter.char} key={'char_' + letter.char} className='Keyboard-char-enter' onClick={e=>handleKeyup(e.target.innerHTML)}>{letter.char}</div>
                            :
                            letter.char === 'Backspace' ?
                                <div id={'char_' + letter.char} key={'char_' + letter.char} className='Keyboard-char-backspace' onClick={e=>handleKeyup(e.target.id)}><IoBackspaceOutline/></div>
                                :
                                <div id = {'char_' + letter.char} key={'char_' + letter.char} className={'Keyboard-char ' + color} onClick={e=>handleKeyup(e.target.innerHTML)}>{letter.char}</div>
                    )
                })}
